...
function HealthStateBadge({ state }: { state: JvmHealthState }) {
  const config: Record<JvmHealthState, { label: string; className: string }> = {
    healthy: { label: '健康', className: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' },
    degraded: { label: '降级', className: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' },
...
function RiskSignalCard({ signal }: { signal: JvmRiskSignalDto }) {
  const levelColors: Record<string, string> = {
    ...
    critical: 'border-red-500 bg-red-100 dark:border-red-600 dark:bg-red-900/30',
  };
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
          ...
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${summary.helperAttached ? 'bg-green-500' : 'bg-gray-400'}`}></span>
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
          ...
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${summary.jmxConnected ? 'bg-green-500' : 'bg-gray-400'}`}></span>
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
            ...
            {summary.activeJfrRecordings.map((rec) => (
              <div key={rec.id} className="flex items-center justify-between text-xs">
                ...
                <span className={`px-1.5 py-0.5 rounded text-xs ${
                  rec.status === 'running' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                  rec.status === 'completed' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
