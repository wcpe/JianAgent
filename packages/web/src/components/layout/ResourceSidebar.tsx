...
const HEALTH_COLORS: Record<string, string> = {
  healthy: 'bg-success-100 text-success-700 dark:bg-success-700/40 dark:text-success-200',
  degraded: 'bg-warning-100 text-warning-700 dark:bg-warning-700/40 dark:text-warning-200',
  critical: 'bg-danger-100 text-danger-700 dark:bg-danger-700/40 dark:text-danger-200',
...
export function ResourceSidebar() {
  ...
  const { detail, backTo = '/resources', backLabel = t('sidebar.backToResources') } = props;
  const visibleTabs = getVisibleTabs(detail);
  
  const healthClass = HEALTH_COLORS[detail.status.health] ?? HEALTH_COLORS.unknown;
  const healthLabel = t(HEALTH_LABEL_KEYS[detail.status.health] ?? HEALTH_LABEL_KEYS.unknown);
      ...
      >
        ...
        <div className="py-2 border-t border-gray-200 dark:border-gray-800">
          ...
          <button
            ...
            title={collapsed && !floating ? t('nav.logout') : undefined}
          >
