    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {([
          ...
          { label: '磁盘', value: (() => { const d = data.systemMetrics?.disks?.[0]; return d && d.usedPercent != null ? `${d.usedPercent.toFixed(0)}%` : '—'; })(), sub: (() => { const d = data.systemMetrics?.disks?.[0]; return d && d.usedGb != null && d.totalGb != null ? `${d.usedGb.toFixed(1)}/${d.totalGb.toFixed(1)} GB (${d.mount ?? '/'})` : undefined; })(), accent: (() => { const d = data.systemMetrics?.disks?.[0]; return d && d.usedPercent != null && d.usedPercent > 85 ? 'text-danger-600 dark:text-danger-400' : undefined; })() },
        ]).map((m) => (
