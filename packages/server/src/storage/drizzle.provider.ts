...
export class DrizzleProvider implements OnModuleDestroy {
  ...
  private runMigrations(): void {
    ...
    const localValidationRunColumns = this.sqlite
    ...
    const cpApprovalColumns = this.sqlite
      .prepare("PRAGMA table_info('cp_approval_tickets')")
      .all() as Array<{ name: string }>;

    if (!cpApprovalColumns.some((c) => c.name === 'confirmation_code')) {
      this.sqlite.exec("ALTER TABLE cp_approval_tickets ADD COLUMN confirmation_code TEXT NOT NULL DEFAULT ''; ");
    }
    if (!cpApprovalColumns.some((c) => c.name === 'expires_at')) {
      this.sqlite.exec("ALTER TABLE cp_approval_tickets ADD COLUMN expires_at INTEGER NOT NULL DEFAULT 0; ");
    }

    const cpOperationJobColumns = this.sqlite
      .prepare("PRAGMA table_info('cp_operation_jobs')")
      .all() as Array<{ name: string }>;

    if (!cpOperationJobColumns.some((c) => c.name === 'validation_plan_id')) {
      this.sqlite.exec("ALTER TABLE cp_operation_jobs ADD COLUMN validation_plan_id TEXT;");
    }
    if (!cpOperationJobColumns.some((c) => c.name === 'validation_run_id')) {
      this.sqlite.exec("ALTER TABLE cp_operation_jobs ADD COLUMN validation_run_id TEXT;");
    }

    const serverConfigColumns = this.sqlite
      .prepare("PRAGMA table_info('server_configs')")
      .all() as Array<{ name: string }>;

    if (!serverConfigColumns.some((column) => column.name === 'is_active')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN is_active INTEGER NOT NULL DEFAULT 0;");
    }
    if (!serverConfigColumns.some((column) => column.name === 'host')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN host TEXT NOT NULL DEFAULT 'localhost';");
    }
    if (!serverConfigColumns.some((column) => column.name === 'port')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN port INTEGER NOT NULL DEFAULT 25565;");
    }
    if (!serverConfigColumns.some((column) => column.name === 'server_type')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN server_type TEXT NOT NULL DEFAULT 'managed';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_host')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_host TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_port')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_port INTEGER NOT NULL DEFAULT 22;");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_username')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_username TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_auth_type')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_auth_type TEXT NOT NULL DEFAULT 'password';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_password')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_password TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_key_path')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_key_path TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ssh_passphrase')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ssh_passphrase TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'server_dir')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN server_dir TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'probe_version')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN probe_version TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'logs_path')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN logs_path TEXT NOT NULL DEFAULT 'logs';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'runtime_id')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN runtime_id TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'pre_start_command')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN pre_start_command TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'post_start_command')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN post_start_command TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'pre_stop_command')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN pre_stop_command TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'post_stop_command')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN post_stop_command TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'script_timeout_ms')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN script_timeout_ms INTEGER NOT NULL DEFAULT 30000;");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ready_pattern')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ready_pattern TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'ready_timeout_ms')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN ready_timeout_ms INTEGER NOT NULL DEFAULT 120000;");
    }
    if (!serverConfigColumns.some((c) => c.name === 'server_group')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN server_group TEXT NOT NULL DEFAULT '';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'tags')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN tags TEXT NOT NULL DEFAULT '[]';");
    }
    if (!serverConfigColumns.some((c) => c.name === 'description')) {
      this.sqlite.exec("ALTER TABLE server_configs ADD COLUMN description TEXT NOT NULL DEFAULT '';");
    }

    const templateColumns = this.sqlite
      .prepare("PRAGMA table_info('server_start_templates')")
      .all() as Array<{ name: string }>;
    
    if (!templateColumns.some((c) => c.name === 'runtime_id')) {
      this.sqlite.exec("ALTER TABLE server_start_templates ADD COLUMN runtime_id TEXT NOT NULL DEFAULT '';");
    }
    if (!templateColumns.some((c) => c.name === 'template_group')) {
      this.sqlite.exec("ALTER TABLE server_start_templates ADD COLUMN template_group TEXT NOT NULL DEFAULT '';");
    }
    if (!templateColumns.some((c) => c.name === 'template_description')) {
      this.sqlite.exec("ALTER TABLE server_start_templates ADD COLUMN template_description TEXT NOT NULL DEFAULT '';");
    }

    const metricColumns = this.sqlite
      .prepare("PRAGMA table_info('metric_snapshots')")
      .all() as Array<{ name: string }>;

    if (!metricColumns.some((c) => c.name === 'max_memory_mb')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN max_memory_mb REAL;');
    }
    if (!metricColumns.some((c) => c.name === 'world_count')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN world_count INTEGER;');
    }
    if (!metricColumns.some((c) => c.name === 'max_players')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN max_players INTEGER;');
    }
    if (!metricColumns.some((c) => c.name === 'plugin_count')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN plugin_count INTEGER;');
    }
    if (!metricColumns.some((c) => c.name === 'player_details')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN player_details TEXT;');
    }
    if (!metricColumns.some((c) => c.name === 'plugin_details')) {
      this.sqlite.exec('ALTER TABLE metric_snapshots ADD COLUMN plugin_details TEXT;');
    }

    const fileTasksTableExists = this.sqlite
      .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='file_tasks' LIMIT 1")
      .get() as { 1: number } | undefined;

    if (fileTasksTableExists) {
      const fileTaskColumns = this.sqlite
        .prepare("PRAGMA table_info('file_tasks')")
        .all() as Array<{ name: string }>;

      if (!fileTaskColumns.some((c) => c.name === 'attempt')) {
        this.sqlite.exec('ALTER TABLE file_tasks ADD COLUMN attempt INTEGER NOT NULL DEFAULT 1;');
      }
      if (!fileTaskColumns.some((c) => c.name === 'max_attempts')) {
        this.sqlite.exec('ALTER TABLE file_tasks ADD COLUMN max_attempts INTEGER NOT NULL DEFAULT 1;');
      }
      if (!fileTaskColumns.some((c) => c.name === 'retryable')) {
        this.sqlite.exec('ALTER TABLE file_tasks ADD COLUMN retryable INTEGER NOT NULL DEFAULT 0;');
      }
      if (!fileTaskColumns.some((c) => c.name === 'resume_token')) {
        this.sqlite.exec('ALTER TABLE file_tasks ADD COLUMN resume_token TEXT;');
      }
      if (!fileTaskColumns.some((c) => c.name === 'failure_class')) {
        this.sqlite.exec('ALTER TABLE file_tasks ADD COLUMN failure_class TEXT;');
      }
    }
