...
export class MetricsRealtimeWiring implements OnModuleInit, OnModuleDestroy {
  ...
  private systemHandler: ((snapshot: any) => void) | null = null;
  ...
  onModuleInit(): void {
    ...
    this.jmxHandler = (snapshot: JmxMetricSnapshotDto) => {
      this.realtimeGateway.broadcastRaw(
        createWsMessage(WsChannel.RESOURCE_METRIC_SUMMARY, {
          type: 'jmx',
          snapshot,
        }),
      );
    ...
    this.systemHandler = (snapshot: any) => {
      this.realtimeGateway.broadcastRaw(
        createWsMessage(WsChannel.SYSTEM_METRICS, { type: 'system', snapshot }),
      );
    };
    this.systemMetrics.on('system.snapshot', this.systemHandler);
  }
  ...
  onModuleDestroy(): void {
    ...
    if (this.systemHandler) this.systemMetrics.removeListener('system.snapshot', this.systemHandler);
  }
}
